...
class Id<T>(private val data: T) {
    ...
    infix fun <T2> `*?*`(f: (T) -> T2): Id<T2> { return fmap(f) }
    fun <T2> ap(f: Id<(T) -> T2>): Id<T2> { return Id(f.data(data)) }
    infix fun <T2> `***`(f: Id<(T) -> T2>): Id<T2> { return ap(f) }
    fun <T2> bind(f: (T) -> Id<T2>): Id<T2> { return f(data) }
    infix fun <T2> `**=`(f: (T) -> Id<T2>): Id<T2> { return bind(f) }

    companion object {
        fun <T> pure(data: T): Id<T> { return Id(data) }
    }
}